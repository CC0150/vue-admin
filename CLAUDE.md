...
```
npm run dev          # 启动开发服务器（自动打开浏览器，Mock API 已启用）
npm run build:test   # 类型检查 + 构建（测试/mock 模式）
npm run build:pro    # 类型检查 + 构建（生产模式）
...
- 开发模式下，匹配 `/api` 的请求会被代理到 `http://127.0.0.1:10086`（可通过 `.env.development` 配置）。
- `vite-plugin-mock` 在开发时从 `mock/user.ts` 提供 Mock API 接口。
