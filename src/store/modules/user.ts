...
// 过滤当前用户可访问的异步路由（无副作用，不修改原数组）
const filterAsyncRoutes = (asyncRoutes: RouteRecordRaw[], routes: string[]): RouteRecordRaw[] => {
  return asyncRoutes.flatMap((item: any) => {
    if (!routes.includes(item.name)) return [];
...
export const useUserStore = defineStore("user", () => {
  ...
  const getUserInfo = async () => {
    ...
    if (res.code === 200) {
      ...
      // 过滤异步路由（filterAsyncRoutes 已返回新对象，无需深拷贝）
      const userAsyncRoutes = filterAsyncRoutes(asyncRoute, res.data.routes);
      // 更新菜单路由-用于展示用户菜单
